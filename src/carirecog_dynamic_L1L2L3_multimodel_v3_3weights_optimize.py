...
def main(args):
    ...
    if args.evaluate_express:
        ...
        ##################################################################################################################
        logits_lossweights = slim.fully_connected(layer1_lossweights_embedings, 3, activation_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
            ...
            while epoch < args.max_nrof_epochs:
                ...
                learning_rate_\
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op,
                            ...
                            acc_v2c, acc_c2v, best_acc_c2v, best_acc_v2c, acc_mixrecog, best_acc_mixrecog, train_op_weights,
                            loss_for_weights, grads_weights, grads_clip__weights, layer1_lossweights)
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          ...
          best_acc_mixrecog, train_op_weights, loss_for_weights, grads_weights, grads_clip__weights, layer1_lossweights):
    batch_number = 0
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            _, loss_for_weights_, grads_weights_, grads_clip__weights_, layer1_lossweights_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_mean_cari, loss_cari_percentage, train_op_weights, loss_for_weights,
                            grads_weights, grads_clip__weights, layer1_lossweights, summary_op], feed_dict=feed_dict)
            ...
            loss_cari_percentage_, _, loss_for_weights_, grads_weights_, grads_clip__weights_, layer1_lossweights_ \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
                            ...
                            loss_expr_percentage, logits_lossweights_embedings, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari, loss_cari_percentage, train_op_weights, loss_for_weights,
                            grads_weights, grads_clip__weights, layer1_lossweights], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        Loss_verif %2.3f Loss_visual-recog %2.3f Loss_cari-recog %2.3f CrossEntropy_verif %2.3f CrossEntropy_visual-recog %2.3f CrossEntropy_cari-recog %2.3f\n \
        weight_verif: %2.3f weight_visual-recog: %2.3f weight_cari-recog: %2.3f  exp(logits_lossweights_embedding): %f %f %f loss_for_weights: %f \n \
        RegLoss %2.3f CenterLoss-verif_l2 %2.3f softmaxAcc_cari-visual %2.3f softmaxAcc_visual-id %2.3f softmaxAcc_cari-id %2.3f lr: %e\n' %
