...
def main(args):
    ...
    # list_test = list(set(image_list_test))
    # list_train_idx = []
    # for i, img in enumerate(image_list_all):
    #     if not img in list_test:
    #         list_train_idx += [i]
    ...
    image_list_test, label_list_id_test, nrof_classes_test, label_verif, label_cari_test\
        = facenet_ext.get_image_paths_and_labels_cavi_fromfile(args.test_pairs, args.data_dir)

    ## downsampling the test samples
    down_sample = 4
    xx = zip(image_list_test[0::2], image_list_test[1::2], label_list_id_test[0::2], label_list_id_test[1::2], label_cari_test[0::2], label_cari_test[1::2])
    yy = xx[0::down_sample]
    ...
    xx = zip(image_list_test, label_list_id_test, label_cari_test)
    yy = list(set(xx))
    zz = zip(*yy)
    ...
    if args.evaluate_express:
        ...
        #logits_lossweights = slim.fully_connected(input_lossweights, 3, activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
        logits_lossweights = slim.fully_connected(input_lossweights_embedings, 3, activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
        softmax_lossweights =tf.nn.softmax(logits_lossweights)
        loss_verif_percentage = 1-tf.reduce_mean(softmax_lossweights[:, 0])+args.loss_weight_base1
        loss_expr_percentage = 1-tf.reduce_mean(softmax_lossweights[:, 1])+args.loss_weight_base2
        loss_cari_percentage = 1-tf.reduce_mean(softmax_lossweights[:, 2])+args.loss_weight_base3

        ##################### Full loss ###################################

        # weight_fullcrossentropy = tf.add_n([tf.multiply(loss_verif_percentage+args.loss_weight_base1, loss_verif)]
        #                                    +[tf.multiply(loss_expr_percentage+args.loss_weight_base2, loss_expr)]
        #                                    +[tf.multiply(loss_cari_percentage + args.loss_weight_base3, loss_cari)],
            ...
            while epoch < args.max_nrof_epochs:
                ...
                learning_rate_\
                    = train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op,
                            ...
                            best_authen_exprecog, loss_verif_percentage, loss_expr_percentage, epoch_current, logits_lossweights,
                ...
                if (epoch % 1 == 0):
                     if args.expr_pairs:
                         acc_expr_paris, val_expr_paris, far_expr_paris = evaluate(sess, enqueue_op, image_paths_placeholder, labels_id_placeholder,
                                                  ...
                                                  log_dir, step, summary_writer, args.evaluate_mode,
                                                  keep_probability_placeholder, 'cari-verif-pairs', best_acc_faceverif_expr, args)
                    ...
                    if args.evaluate_express:
                        acc_expression, each_expr_acc, exp_cnt, expredict_cnt,  express_probs_confus_matrix, express_recog_images\
                            = evaluate_expression(sess, batch_size_placeholder, logits_expr,
                                                  ...
                                                  best_acc_exprecog, phase_train_placeholder_cari, 'visual')
                ...
                if not (epoch % 1):
                    acc_cari, each_cari_acc, cari_cnt, caripredict_cnt, cari_probs_confus_matrix, cari_recog_images \
                        = evaluate_expression(sess, batch_size_placeholder, logits_cari,
                                              ...
                                              log_dir, step, summary_writer, keep_probability_placeholder, input_queue,
                                              phase_train_placeholder_expression, phase_train_placeholder, args,
                                              best_acc_carirecog, phase_train_placeholder_cari, 'cari')
                ...
                #     f.write('loss_verif: %2.3f  loss_expr: %2.3f  crossentropy: %2.3f  regloss: %2.3f  centerloss: %2.3f  verifAcc: %2.3f  lr:%e \n' % (loss_verif_, loss_expr_, cross_entropy_mean_expr_, Reg_loss,  center_loss_, verifacc, learning_rate_))
                #     line = ''
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          ...
          best_authen_exprecog, loss_verif_percentage, loss_expr_percentage, epoch_current, logits_lossweights, label_list_cari,
          labels_cari_placeholder, phase_train_placeholder_cari, softmax_acc_cari, loss_cari, cross_entropy_mean_cari,
          loss_cari_percentage, acc_cari, best_acc_carirecog):
    batch_number = 0
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_verif_percentage_, \
            loss_expr_percentage_,logits_lossweights_, softmax_acc_cari_, loss_cari_, cross_entropy_mean_cari_, \
            loss_cari_percentage_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem, loss_verif_percentage,
                            loss_expr_percentage, logits_lossweights, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari, loss_cari_percentage, summary_op], feed_dict=feed_dict)
            ...
            loss_cari_percentage_ \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem, loss_verif_percentage,
                            loss_expr_percentage, logits_lossweights, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari, loss_cari_percentage], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        # for i, grad in enumerate(grads_expr_):
        ...
        # print('\n')
        # for i, grad_clip in enumerate(grads_clip_expr_):
        ...
        print('Epoch: [%d][%d][%d/%d]\tTime %.3f\tLoss_cari-visual-verif %2.3f\tLoss_visual-id %2.3f\tLoss_cari-id %2.3f\tCrossEntropy_verif %2.3f \
        \tCrossEntropy_visual-id %2.3f\tCrossEntropy_cari-id %2.3f\tRegLoss %2.3f\tCenterLoss-verif_l2 %2.3f\tsoftmaxAcc_cari-visual %2.3f\tsoftmaxAcc_visual-id %2.3f \
        \tsoftmaxAcc_cari-id %2.3f \tlr: %e\tloss_verif_percentage: %f\tloss_visual_percentage: %f\tloss_cari_percentage: %f \tlogits_lossweights: %f %f %f' %
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, loss_cari_, cross_entropy_mean_verif_, \
             cross_entropy_mean_expr_, cross_entropy_mean_cari_, np.sum(reg_loss), prelogits_center_loss_verif_, softmax_acc_verif_,  \
             softmax_acc_expr_, softmax_acc_cari_, learning_rate_,  np.mean(loss_verif_percentage_), np.mean(loss_expr_percentage_),np.mean(loss_cari_percentage_),
             np.exp(np.mean(logits_lossweights_[:,0])), np.exp(np.mean(logits_lossweights_[:,1])), np.exp(np.mean(logits_lossweights_[:,2]))))
        ...
        print('Visual image recognition : test_visual_acc %2.3f, best_visual_acc %2.3f'%(acc_expression, best_acc_exprecog))
        print('Caricature image recognition : test_cari_acc %2.3f, best_cari_acc %2.3f'%(acc_cari, best_acc_carirecog))
        ...
        with open(os.path.join(log_dir, 'percentage_full_loss.txt'), 'at') as f:
            f.write('%f %f %f\n' % (np.mean(loss_verif_percentage_), np.mean(loss_expr_percentage_), np.mean(loss_cari_percentage_)))
        with open(os.path.join(log_dir, 'loss.txt'), 'at') as f:
            f.write('%f %f %f %f %f %f %f\n' % (loss_verif_, loss_expr_, loss_cari_, cross_entropy_mean_verif_,cross_entropy_mean_expr_,cross_entropy_mean_cari_, prelogits_center_loss_verif_))
    ...
    return step, softmax_acc_verif_, softmax_acc_expr_, loss_verif_, loss_expr_, cross_entropy_mean_verif_, cross_entropy_mean_expr_, np.sum(reg_loss),  prelogits_center_loss_verif_, acc, learning_rate_
...
def evaluate_expression(sess,
             ...
             best_acc_exprecog, phase_train_placeholder_cari, imgtype):
    start_time = time.time()
    # Run forward pass to calculate embeddings
    ...
    print('%d %s recognition accuracy is: %f' % (nrof_expression, imgtype,test_expr_acc))
