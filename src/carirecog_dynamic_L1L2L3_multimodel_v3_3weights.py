...
def main(args):
    ...
    if args.evaluate_express:
        ...
        ##################### Full loss ###################################
        loss_verif_percentage = tf.cast(0.40, tf.float32)
        loss_expr_percentage = tf.cast(0.35, tf.float32)
        loss_cari_percentage = tf.cast(0.25, tf.float32)
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_verif_percentage_, \
            loss_expr_percentage_,logits_lossweights_, softmax_acc_cari_, loss_cari_, cross_entropy_mean_cari_, loss_cari_percentage_,\
            summary_str \
        ...
        \tsoftmaxAcc_cari-id %2.3f \tlr: %e\tloss_verif_percentage: %f\tloss_visual_percentage: %f\tloss_cari_percentage: %f \tlogits_lossweights: %f %f' %
