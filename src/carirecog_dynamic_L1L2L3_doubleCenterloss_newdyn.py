...
def main(args):
    ...
    image_list_test, label_list_id_test, nrof_classes_test, label_verif\
        = facenet_ext.get_image_paths_and_labels_cavi_fromfile(args.test_pairs, args.data_dir)
    down_sample = 4
    xx = zip(image_list_test[0::2], image_list_test[1::2])
    yy = xx[0::down_sample]
    ...
    for y in yy:
        list_tmp.append(y[0])
        list_tmp.append(y[1])
    image_list_test = list_tmp
    xx = zip(label_list_id_test[0::2], label_list_id_test[1::2])
    yy = xx[0::down_sample]
    #label_list_id_test = zip(*yy)[0] + zip(*yy)[1]
    list_tmp = []
    for y in yy:
        list_tmp.append(y[0])
        list_tmp.append(y[1])
    label_list_id_test = list_tmp
    label_verif = label_verif[0::down_sample]

    xx = zip(image_list_test, label_list_id_test)
    yy = list(set(xx))
    zz = zip(*yy)
    ...
    if args.evaluate_express:
        ...
        #loss_verif = tf.add_n([cross_entropy_mean_verif]+[args.center_loss_factor*prelogits_center_loss_verif], name='loss_verif')
        ...
        embeddings_expression = tf.nn.l2_normalize(prelogits, 1, 1e-10, name='embeddings')
        ...
        prelogits_expression_center_loss, prelogits_expression_center_loss_n, centers_expression, _, centers_cts_batch_reshape_expression, diff_mean_expression \
            = metrics_loss.center_loss(embeddings_expression, label_expr_batch, args.center_loss_alfa, nrof_classes)
        ...
        logits_lossweights = slim.fully_connected(input_lossweights, 2, activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
        softmax_lossweights =tf.nn.softmax(logits_lossweights)
        loss_verif_percentage = softmax_lossweights[:, 0]
        loss_expr_percentage = softmax_lossweights[:, 1]

        ##################### Full loss ###################################
        #### loss_full = loss_verif_percentage*loss_verif + loss_expr_percentage*loss_expr
        #weight_fullcrossentropy = tf.add_n([tf.multiply(loss_verif_percentage+args.loss_weight_base, cross_entropy_verif)]+[tf.multiply(loss_expr_percentage+args.loss_weight_base2, cross_entropy_expr)], name='weight_fullcrossentropy')
        ...
        weight_fullcrossentropy = tf.add_n([tf.multiply(1-loss_verif_percentage, loss_verif_n)]+[tf.multiply(1-loss_expr_percentage, loss_expr_n)], name='weight_fullcrossentropy')
                    ...
                    if args.evaluate_express:
                        acc_expression, each_expr_acc, exp_cnt, expredict_cnt,  express_probs_confus_matrix, express_recog_images\
                            = evaluate_expression(sess, enqueue_op, image_paths_placeholder, labels_id_placeholder,
                                                  ...
                                                  label_expr_batch, image_list_test_id, label_list_id_test, 100,
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
    ...
    sess.run(enqueue_op, {image_paths_placeholder: image_paths_array, labels_id_placeholder: labels_id_array, labels_expr_placeholder: labels_id_array})
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % 100 == 0):
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_verif_percentage_, \
            loss_expr_percentage_,logits_lossweights_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem, loss_verif_percentage,
                            loss_expr_percentage, logits_lossweights, summary_op], feed_dict=feed_dict)
            ...
            label_batch_expr_, image_batch_, learning_rate_, softmax_acc_verif_, \
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, loss_verif_percentage_, \
            loss_expr_percentage_, logits_lossweights_ \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
                            cross_entropy_verif, diff_mean,centers, train_op_mainstem, loss_verif_percentage,
                            loss_expr_percentage, logits_lossweights], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        print('Epoch: [%d][%d][%d/%d]\tTime %.3f\tLoss_verif %2.3f\tLoss_expr %2.3f\tCrossEntropy_verif %2.3f \
        \tCrossEntropy_expr %2.3f\tRegLoss %2.3f\tCenterLoss_l2 %2.3f\tsoftmax_acc_verif %2.3f\tsoftmax_acc_expr %2.3f \
        \tlr: %e\tloss_verif_percentage: %f\tloss_expr_percentage: %f \t logits_lossweights: %f %f' %
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, cross_entropy_mean_verif_, \
             cross_entropy_mean_expr_, np.sum(reg_loss), prelogits_center_loss_verif_, softmax_acc_verif_,  \
             softmax_acc_expr_, learning_rate_,  np.mean(loss_verif_percentage_), np.mean(loss_expr_percentage_),
             np.exp(logits_lossweights_[0,0]), np.exp(logits_lossweights_[0,1])))
        ...
        print('Face expression: test_expr_acc %2.3f, best_acc_exprecog %2.3f'%(acc_expression, best_acc_exprecog))
        ...
        with open(os.path.join(log_dir, 'loss.txt'), 'at') as f:
            f.write('%f\t%f\t%f\t%f\n' % (loss_verif_, loss_expr_, cross_entropy_mean_verif_, prelogits_center_loss_verif_))
...
def parse_arguments(argv):
    ...
    parser.add_argument('--loss_weight_base', type=float,
