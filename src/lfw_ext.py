...
def evaluate(embeddings, actual_issame, nrof_folds=10, far=0.01):
    ...
    tprs, fprs, accuracy, fp_idx, fn_idx, best_threshold_acc = facenet_ext.calculate_roc(thresholds, embeddings1, embeddings2,
        np.asarray(actual_issame), nrof_folds=nrof_folds)
    thresholds = np.arange(0, 4, 0.001)
    val_mean, val_std = facenet_ext.calculate_val(thresholds, embeddings1, embeddings2,
        np.asarray(actual_issame),far, nrof_folds=nrof_folds)
    ...
    return tprs, fprs, accuracy, val_mean, val_std, fp_idx, fn_idx, best_threshold_acc
