...
def main(args):
    ...
    #########################   webcari    ##########################
    image_list, label_list_id, label_list_cari, label_verif, nrof_classes \
        = facenet_ext.get_image_paths_and_labels_webcari_rank_train(args.train_pairs, args.data_dir)
    ...
    if args.evaluate_express:
        ...
        ##################################################################################################################
        logits_lossweights = slim.fully_connected(input_lossweights, 3, activation_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
        ...
        train_op_weights, grads_weights, grads_clip__weights = train_BP.train(loss_for_weights, global_step, args.optimizer,
            learning_rate*10, args.moving_average_decay, update_gradient_vars_weights, summary_op, args.log_histograms)
...
def train(args, sess, epoch, image_list, label_list_id, index_dequeue_op, enqueue_op, image_paths_placeholder,
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            loss_cari_percentage_, _, loss_for_weights_, grads_weights_, grads_clip__weights_,\
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
