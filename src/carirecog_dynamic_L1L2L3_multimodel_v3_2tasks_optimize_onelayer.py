...
def main(args):
    ...
    if args.evaluate_express:
        ...
        ### mapping logits_lossweights_embedings to [0-3] in order to make the weight in the range of e^[0-3]
        logits_lossweights_embedings = logits_lossweights_embedings*2
        ...
        loss_expr_percentage = tf.reduce_mean(softmax_lossweights[:, 0])+args.loss_weight_base2
        loss_cari_percentage = tf.reduce_mean(softmax_lossweights[:, 1])+args.loss_weight_base3
        #loss_cari_percentage = 1.0

        ###################### alpha weights optimization loss  ##########################
        sigma = 0.01
        loss_for_weights = tf.add_n([tf.divide(loss_expr_percentage, loss_expr+sigma)]
        ...
        weight_fullcrossentropy = tf.add_n([tf.multiply(loss_expr_percentage, loss_expr)]
                                           +[tf.multiply(loss_cari_percentage, loss_cari)],
                                           name='weight_fullcrossentropy')

        # weight_fullcrossentropy = tf.add_n([tf.multiply(0.05, loss_expr)]
        #                                    +[tf.multiply(0.95, loss_cari)],
        #                                    name='weight_fullcrossentropy')
        ...
        train_op_weights, grads_weights, grads_clip__weights = train_BP.train(loss_for_weights, global_step, args.optimizer,
            learning_rate*20, args.moving_average_decay, update_gradient_vars_weights, summary_op, args.log_histograms)
    ...
    while batch_number < args.epoch_size:
        ...
        RegLoss %2.4f CenterLoss-verif_l2 %2.4f softmaxAcc_cari-visual %2.4f softmaxAcc_visual-id %2.4f softmaxAcc_cari-id %2.4f lr: %e\n' %
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, loss_cari_, cross_entropy_mean_verif_, \
             ...
             np.exp(np.mean(logits_lossweights_embedings_[:,0])), np.exp(np.mean(logits_lossweights_embedings_[:,1])), np.exp(np.mean(logits_lossweights_embedings_[:,1])), loss_for_weights_, \
