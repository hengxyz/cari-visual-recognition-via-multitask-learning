...
def main(args):
    ...
    image_list_test_id_visual = list(compress(image_list_test, filter))
    ...
    image_list_test_id_cari = list(compress(image_list_test, filter))
    ...
    if args.evaluate_express:
        ...
        #logits_expr = slim.fully_connected(logits_0, len(set(label_list_id)), activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits', reuse=False)
        ...
        loss_verif_percentage_ = args.loss_weight1
        loss_expr_percentage_ = args.loss_weight2
        loss_cari_percentage_ = args.loss_weight3

        weight_fullcrossentropy = tf.add_n([tf.multiply(loss_verif_percentage_, loss_verif)]
                                            +[tf.multiply(loss_expr_percentage_, loss_expr)]
                                           +[tf.multiply(loss_cari_percentage_, loss_cari)],
            ...
            while epoch < args.max_nrof_epochs:
                ...
                learning_rate_, loss_verif_weight, loss_expr_weight, loss_cari_weight, loss_cari_, cross_entropy_mean_cari_, prelogits_center_loss_verif_\
                    = train(args, sess, epoch, image_list, label_list_id, index_dequeue_op, enqueue_op,
                            ...
                            best_authen_exprecog, loss_verif_percentage_, loss_expr_percentage_, epoch_current, logits_lossweights_embedings,
                            label_list_cari, labels_cari_placeholder, phase_train_placeholder_cari, softmax_acc_cari,
                            loss_cari, cross_entropy_mean_cari, loss_cari_percentage_, acc_cari, best_acc_carirecog,
                            acc_v2c, acc_c2v, best_acc_c2v, best_acc_v2c, acc_mixrecog, best_acc_mixrecog, train_op_weights,
                            loss_for_weights, grads_weights, grads_clip__weights, f_weights, f_loss)
...
def train(args, sess, epoch, image_list, label_list_id, index_dequeue_op, enqueue_op, image_paths_placeholder,
          ...
          best_authen_exprecog, loss_verif_percentage_, loss_expr_percentage_, epoch_current, logits_lossweights_embedings, label_list_cari,
          labels_cari_placeholder, phase_train_placeholder_cari, softmax_acc_cari, loss_cari, cross_entropy_mean_cari,
          loss_cari_percentage_, acc_cari, best_acc_carirecog, acc_v2c, acc_c2v, best_acc_c2v, best_acc_v2c, acc_mixrecog,
          best_acc_mixrecog, train_op_weights, loss_for_weights, grads_weights, grads_clip__weights, f_weights, f_loss):
    batch_number = 0
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, \
            logits_lossweights_embedings_, softmax_acc_cari_, loss_cari_, cross_entropy_mean_cari_,\
            _, loss_for_weights_, grads_weights_, grads_clip__weights_, summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem,
                            logits_lossweights_embedings, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari, train_op_weights, loss_for_weights,
                            grads_weights, grads_clip__weights, summary_op], feed_dict=feed_dict)
            ...
            softmax_acc_expr_, cross_entropy_verif_, diff_mean_, centers_, _, \
            logits_lossweights_embedings_, softmax_acc_cari_, loss_cari_, cross_entropy_mean_cari_, \
             _, loss_for_weights_, grads_weights_, grads_clip__weights_,\
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            prelogits_center_loss_verif, cross_entropy_mean_verif, cross_entropy_mean_expr,
                            centers_cts_batch_reshape, logits_id, logits_expr, label_batch_id, label_batch_expr,
                            image_batch, learning_rate, softmax_acc_verif, softmax_acc_expr,
                            ...
                            logits_lossweights_embedings, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari, train_op_weights, loss_for_weights,
                            grads_weights, grads_clip__weights], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        # nrof_expression = len(set(label_list_id))
...
def parse_arguments(argv):
    ...
    parser.add_argument('--loss_weight1', type=float,
                        help='The base of the weight of the third sub-loss in the full loss.', default=0)
    parser.add_argument('--loss_weight2', type=float,
                        help='The base of the weight of the third sub-loss in the full loss.', default=0)
    parser.add_argument('--loss_weight3', type=float,
                        help='The base of the weight of the third sub-loss in the full loss.', default=0)
