...
def main(args):
    ...
    list_class_test = list(set(label_list_test))
    list_class_test = [x.upper() for x in list_class_test]

    image_paths_train, image_paths_test, labels_train, labels_test, labels_cari_train, labels_cari_test\
        = facenet_ext.select_image_paths_and_labels_cavi(args.data_dir, list_class_test, args.ifold, args.nfold)
    ...
    image_list_test_id = image_paths_test
    label_list_id_test = labels_test
    ...
    if args.evaluate_express:
        ...
        logits_lossweights = slim.fully_connected(input_lossweights_embedings, 2, activation_fn=tf.nn.relu,weights_initializer=tf.truncated_normal_initializer(stddev=0.1), weights_regularizer=slim.l2_regularizer(args.weight_decay),scope='Logits_lossweights', reuse=False)
                ...
                if not (epoch % 1):
                    acc_v2c, each_v2c_acc, v2c_cnt, caripredict_v2c, v2c_probs_confus_matrix, v2c_recog_images \
                        = evaluate_expression(sess, batch_size_placeholder, logits_cari,
                                              ...
                                              log_dir, step, summary_writer, keep_probability_placeholder, input_queue,
                                              phase_train_placeholder_expression, phase_train_placeholder, args,
                                              best_acc_v2c, phase_train_placeholder_cari, 'visual2cari')
                ...
                if not (epoch % 1 ):
                    acc_c2v, each_c2v_acc, c2v_cnt, c2vpredict_cnt,  c2v_probs_confus_matrix, c2v_recog_images\
                        = evaluate_expression(sess, batch_size_placeholder, logits_expr,
                                              ...
                                              log_dir, step, summary_writer, keep_probability_placeholder,input_queue,
                                              phase_train_placeholder_expression, phase_train_placeholder, args,
                                              best_acc_c2v, phase_train_placeholder_cari, 'cari2visual')
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
    ...
    while batch_number < args.epoch_size:
        ...
        if (batch_number % args.epoch_size == 0):
            ...
            summary_str \
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_mean_cari, summary_op], feed_dict=feed_dict)
            ...
            loss_expr_percentage_, logits_lossweights_, softmax_acc_cari_, loss_cari_, cross_entropy_mean_cari_\
                = sess.run([loss_verif, loss_expr, global_step, regularization_losses,
                            ...
                            cross_entropy_verif, diff_mean, centers, train_op_mainstem, loss_verif_percentage,
                            loss_expr_percentage, logits_lossweights, softmax_acc_cari, loss_cari,
                            cross_entropy_mean_cari], feed_dict=feed_dict)
        print("step %d"%step)
        ...
        \tsoftmaxAcc_cari-id %2.3f \tlr: %e\tloss_verif_percentage: %f\tloss_visual_percentage: %f\tloss_cari_percentage: 1.0 \tlogits_lossweights: %f %f' %
            (epoch, epoch_current, batch_number + 1, args.epoch_size, duration, loss_verif_, loss_expr_, loss_cari_, cross_entropy_mean_verif_, \
             ...
             softmax_acc_expr_, softmax_acc_cari_, learning_rate_,  np.mean(loss_verif_percentage_), np.mean(loss_expr_percentage_),
             np.exp(np.mean(logits_lossweights_[:,0])), np.exp(np.mean(logits_lossweights_[:,1]))))
        ...
        print('C2V image recognition : test_c2v_acc %2.3f, best_c2v_acc %2.3f'%(acc_c2v, best_acc_c2v))
        print('V2C image recognition : test_v2c_acc %2.3f, best_v2c_acc %2.3f'%(acc_v2c, best_acc_v2c))
        print('Mix image recognition : test_mix_acc %2.3f, best_mix_acc %2.3f' % (acc_mixrecog, best_acc_mixrecog))
...
def evaluate(sess, enqueue_op, image_paths_placeholder, labels_id_placeholder, labels_expr_placeholder, phase_train_placeholder, batch_size_placeholder, 
    ...
    with open(os.path.join(log_dir,dataset+'_result.txt'),'at') as f:
        f.write('%d\t%.5f\t%.5f\t%.5f\t%.5f\t%f\n' % (step, np.mean(accuracy), val, far, best_acc, test_id_acc))
